...
-- sales_rep_id, while processed_by only records who typed the order. The two differ
...
--   * processed_by         the raw string, always stored, so nothing is lost to a bad match
...
--
-- NOTE no semicolons in these comments. database/migrate.php splits files on semicolons
-- without stripping comments, so one here would be treated as a statement boundary.
